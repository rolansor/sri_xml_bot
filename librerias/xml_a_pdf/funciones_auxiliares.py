...
def procesar_archivo_xml(file_path):
    # Leer y parsear el archivo XML principal
    tree = ET.parse(file_path)
    root = tree.getroot()
    datos_extras = {}

    # Comprobar si la raíz es 'autorizaciones' y encontrar la última 'autorizacion'
    if root.tag == 'autorizaciones':
        ...
        # Si no, buscar la primera 'autorizacion'
        autorizacion = root

    # Extraer datos extras de la autorización si está disponible
    if autorizacion is not None:
        datos_extras['estado'] = autorizacion.findtext('estado')
        datos_extras['fechaAutorizacion'] = autorizacion.findtext('fechaAutorizacion')
        datos_extras['ambiente'] = autorizacion.findtext('ambiente')

    if autorizacion is not None:
        comprobante_str = autorizacion.findtext('.//comprobante')
        if comprobante_str:
            # Eliminar declaración de codificación del comprobante embebido y parsearlo
            comprobante_str = re.sub(r'^<\?xml[^>]+\?>', '', comprobante_str.strip())
            comprobante_tree = ET.fromstring(comprobante_str)
            # Extraer el tipo de documento del contenido del archivo XML
            ...
            if tipo_doc_element is not None:
                ...
                if tipo_documento:
                    # Procesar el archivo XML según su tipo
                    if tipo_documento == 'facturas':
                        ...
                        if diccionario_documento is not None:
                            # Asignar datos_extras bajo la clave 'datos_extras'
                            diccionario_documento['datos_extras'] = datos_extras
                            return diccionario_documento
                        # Asumiendo que tienes una función similar para comprobantes de retención
                        ...
                        # Asumiendo que tienes una función similar para notas de crédito
                    ...
                    return None
    # En caso de que no se encuentre el comprobante o el tipo no coincida
    return None